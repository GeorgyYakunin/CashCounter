...
public class AddSalesCashOutAdapter extends RecyclerView.Adapter<AddSalesCashOutAdapter.ViewHolder> {
    Context context;
    ...
    public void onBindViewHolder(final ViewHolder viewHolder, final int i) {
        ...
        viewHolder.minusQty.setOnClickListener(new OnClickListener() {
            public void onClick(View view) {
                if (((AddCashInOutItems) AddSalesCashOutAdapter.this.itemsList.get(i)).getQty() > 0) {
                    ((AddCashInOutItems) AddSalesCashOutAdapter.this.itemsList.get(i)).setQty(((AddCashInOutItems) AddSalesCashOutAdapter.this.itemsList.get(i)).getQty() - 1);
        ...
        viewHolder.plusQty.setOnClickListener(new OnClickListener() {
            public void onClick(View view) {
                if (((AddCashInOutItems) AddSalesCashOutAdapter.this.itemsList.get(i)).getQty() < ((AddCashInOutItems) AddSalesCashOutAdapter.this.itemsList.get(i)).getAvailableQty()) {
        ...
        button2.setOnClickListener(new OnClickListener() {
            public void onClick(View view) {
                if (AddSalesCashOutAdapter.this.lastChange == 1) {
                    AddSalesCashOutAdapter.this.chenageQty(i2, numberPicker.getValue());
                    ...
                    if (textInputLayout.getEditText().getText().toString().trim().isEmpty() || parseInt == 0) {
                        AddSalesCashOutAdapter.this.chenageQty(i2, numberPicker.getValue());
                    } else if (parseInt > ((AddCashInOutItems) AddSalesCashOutAdapter.this.itemsList.get(i2)).getAvailableQty()) {
                       // TextInputLayout textInputLayout = textInputLayout;
                        StringBuilder stringBuilder = new StringBuilder();
                        stringBuilder.append("Maximum ");
                        stringBuilder.append(((AddCashInOutItems) AddSalesCashOutAdapter.this.itemsList.get(i2)).getAvailableQty());
                        stringBuilder.append(" qty available");
                        textInputLayout.setError(stringBuilder.toString());
                        textInputLayout.getEditText().requestFocus();
                        return;
                    } else {
                        AddSalesCashOutAdapter.this.chenageQty(i2, parseInt);
                    }
                }
                alertDialog.dismiss();
            }
